...
datatable(events_by_sites[,-1],rownames = FALSE )

```


The following figures shows the dates of sampling events by site. From this plot, some observations are that routine sampling at many sites began around 2000 and sometime around 2004-5, sampling frequency changed at a number of locations.  Riverwatch indicated that this was done in part to conserve and reallocate resources.
...
The following figure certainly suggests that there are  a few clusters of sites that might be evalulated to assess whether they could be considered as the same, thereby increasing representation along the main stem of the Roaring Fork and the Crystal River ( the southwestern tributary).
...
```{r firstPlot, echo=FALSE, fig.cap= 'asdf'}
plot(cars)
```

Note that the `echo = FALSE` parameter was added to the code chunk to prevent printing of the R code that generated the plot. `r figr("firstPlot",  TRUE, TRUE, type = "Figure")`.
...
What you always wanted to know about cars is shown in figure 
