...
filter(small_dat, Characteristic.Name == input$ycol)
    })    
    
    output$plot1 <- renderPlot({
        unit <- unique(selectedData()$Result.Unit)
        p <- ggplot( data = selectedData(), aes(x = date, y = Result.Value)) + geom_point(aes(shape = Sample.Fraction)) +  facet_wrap(~Monitoring.Location.Name, scales = "free_y") + stat_smooth() + ylab(paste("Result", unit)) + labs(title = input$ycol)
        print(p)
    }, height = 1000)
